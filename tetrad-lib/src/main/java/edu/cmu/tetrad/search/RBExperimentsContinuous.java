...
public class RBExperimentsContinuous {
	...
	public static void main(String[] args) throws IOException {
		...
		double alpha = 0.05, numLatentConfounders = 0.2, lower = 0.0, upper = 1.0;
		int numVars = 10, epn = 4, numCases = 1000, numModels = 100, numBootstrapSamples = 500, round = 0;
		String filePath = "/Users/fattanehjabbari/CCD-Project/CS-BN/bsc-cont/",
		...
		RBExperimentsContinuous DFC = new RBExperimentsContinuous();
		DFC.directory = dataPath;
		double[] lv = new double[]{0.2};//, 0.1, 0.2};
		int[] cases = new int[]{2000};//, 2000};
		for (int numCase: cases){
			for (double numLatentConfounder: lv){
				for (int i = 0; i < 10; i++){
					DFC.experiment(numVars, epn, numCase, numModels, numBootstrapSamples, alpha, numLatentConfounder, threshold1,
	...
	public void experiment(int numVars, int epn, int numCases, int numModels, int numBootstrapSamples, double alpha,
		...
		int numLatents = (int) Math.floor(numLatentConfounders * numVars);

		int numEdges = numVars * epn;
		...
		filePath = filePath + "/" + "-Vars" + dag.getNumNodes() + "-Edges" + dag.getNumEdges() + "-H"
				+ numLatentConfounders + "-Cases" + numCases + "-numModels" + numModels + "-BS" + numBootstrapSamples;
		try {
			File dir = new File(filePath);
			dir.mkdirs();
			File file = new File(dir,
					"Results-" + "-Vars" + dag.getNumNodes() + "-Edges" + dag.getNumEdges() + "-H"
...
//		fullData = refineData(fullData);
		...
		IndTestProbabilisticBic testBSC = runRB(data, bscPags, numModels, threshold1);
...
*/
	...
	private void summarize(Graph graph, Graph trueGraph, PrintStream out) {
		...
		GraphUtils.GraphComparison comparison = SearchGraphUtils.getGraphComparison(graph, trueGraph, true);
//		GraphUtils.GraphComparison cmpI = SearchGraphUtils.getGraphComparison(graphI, truePag, true);
	...
	private DataSet createDepDataFiltering(Map<IndependenceFact, Double> H, DataSet data, int numBootstrapSamples,
		...
		for (int b = 0; b < numBootstrapSamples; b++) {
			...
			IndTestProbabilisticBic bsTest = new IndTestProbabilisticBic(bsData);
	...
	private IndTestProbabilisticBic runRB(DataSet data, List<Graph> pags, int numModels, boolean threshold) {
		IndTestProbabilisticBic BSCtest = new IndTestProbabilisticBic(data);
