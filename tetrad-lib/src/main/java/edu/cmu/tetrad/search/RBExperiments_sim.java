...
public class RBExperiments_sim {
	...
	public static void main(String[] args) throws IOException {
		...
		double alpha = 0.05, cutoff = 0.5, edgesPerNode = 6.0, latent = 0.2,lower = 0.0, upper = 1.0;
		int numVars = 10, numCases = 1000, numSim = 3,
				numModels = 100, numBootstrapSamples = 1000;
		...
		double[] lv = new double[]{0.2};//, 0.1, 0.2};
		int[] cases = new int[]{100, 50};
	...
	public void experiment(int numModels ,double alpha, boolean threshold1, boolean threshold2, double cutoff, int numBootstrapSamples,
			int numVars, double edgesPerNode, double latent, int numCases, int numSim, String data_path, long seed, double lower, double upper){
		...
		double[] arrPLD = new double[numSim], arrRLD = new double[numSim], adjPLD = new double[numSim], adjRLD = new double[numSim], 
				addedLD = new double[numSim], removedLD = new double[numSim], reorientedLD = new double[numSim], shdStrictLD = new double[numSim], shdLenientLD = new double[numSim];
		...
		try {
			File dir = new File(data_path+ "/simulation-Fci2/");
			if (!threshold1){
				cutoff = Double.NaN;
			}
			dir.mkdirs();
			String outputFileName = "V"+numVars +"-E"+ edgesPerNode +"-L"+ latent + "-N" + numCases + "-Th1" + threshold1  + "-C" + cutoff +"-Fci" +".csv";
			String logFileName = "V"+numVars +"-E"+ edgesPerNode +"-L"+ latent + "-N" + numCases + "-Th1" + threshold1  + "-C" + cutoff +"-Fci" +".log";
...
//			System.out.println("Dependency graph done!");
			...
			for (NodePair npair: localBNs.keySet()){
				...
				for (Graph ab_g: localBNs.get(npair).keySet()){
					outlog.println("LocalDepGraph(nodes,edges):" + ab_g.getNumNodes() + "," + ab_g.getNumEdges());
					outlog.println("nodes:" + ab_g.getNodeNames());
					for (Edge e: ab_g.getEdges())
						outlog.println(e);
				}
			}
			...
			outlog.println("maxBNI: " + maxBNI);
			outlog.println();
			outlog.println("bscPags: " + bscPags);
			...
			den = (congLD.getArrowsTp()+congLD.getArrowsFp());
			if (den != 0.0){
				arrPLD[s] += (congLD.getArrowsTp() / den);
			}

			den = (congLD.getArrowsTp()+congLD.getArrowsFn());
			if (den != 0.0){
				arrRLD[s] += (congLD.getArrowsTp() / den);
			}


			den = (conAdjGLD.getAdjTp() + conAdjGLD.getAdjFp());
			if (den != 0.0){
				adjPLD[s] += (conAdjGLD.getAdjTp() / den);
			}

			den = (conAdjGLD.getAdjTp() + conAdjGLD.getAdjFn());
			if (den != 0.0){
				adjRLD[s] += (conAdjGLD.getAdjTp() / den);
			}
		...
		printRes(this.out, "POP BSC-LD", numSim, arrPLD, arrRLD, adjPLD, adjRLD, addedLD, removedLD, reorientedLD, shdStrictLD, shdLenientLD);
	...
	private allScores getLnProbsAll(List<Graph> pags, Map<IndependenceFact, Double> H, DataSet data, BayesIm im,
			Graph dep, Map<NodePair, Map<Graph, BayesIm>> localBNs) {
