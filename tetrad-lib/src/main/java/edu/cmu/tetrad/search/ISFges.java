 ...
 * <p>
 *
 * @author Fattaneh Jabbari April 30, 2019
 */
...
//					int[] parentIndices;
//					if (initialGraph != null){
//						Set<Node> parents = new HashSet<>(initialGraph.getParents(y));
//						parentIndices = new int[parents.size()];
//						c=0;
//						for (Node p : parents) {
...
//						if (initialGraph != null){
//							Set<Node> parents2 = new HashSet<>(initialGraph.getParents(x));
//							parentIndices2 = new int[parents2.size()];
//							c = 0;
//							for (Node p : parents2) {
//								parentIndices2[c++] = hashIndices.get(p);
//							}
//						}
//						else{
//							parentIndices2 = new int[0];
//						}
						...
						bump2 = score.localScoreDiff(child, parent, new int[0], populationParentIndices2, populationChildrenIndices2);
	...
	private double insertEval(Node x, Node y, Set<Node> t, Set<Node> naYX,
		...
		//        System.out.println("INSIDE insertEval");
	...
	private double deleteEval(Node x, Node y, Set<Node> diff, Set<Node> naYX,
		...
		set.addAll(graph.getParents(y));
		set.remove(x);
		//        System.out.println("INSIDE deleteEval");
		return -scoreGraphChange(y, set, x, hashIndices);
	}
	...
	public double scoreDag(Graph dag) {
		...
		for (Node y : dag.getNodes()) {
			...
			//			System.out.println("---------------");
			double ls = score.localScore(yIndex, parentIndices, populationParentIndices, populationChildrenIndices);
